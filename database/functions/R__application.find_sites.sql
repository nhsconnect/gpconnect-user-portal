	...
	select * from (
		select * from (
			select
				...
				json_object_agg(sa.site_attribute_name, sa.site_attribute_value) site_attributes
			from
		...
		where
			...
			(_site_name is null or a.site_attributes->>_site_name_attribute_name ~* _site_name) and
			(_ccg_ods_code is null or position(_ccg_ods_code in a.site_attributes->>_ccg_ods_code_attribute_name) > 0) and
			(_ccg_name is null or position(_ccg_name in a.site_attributes->>_ccg_name_attribute_name) > 0)
			) b
		where case 
